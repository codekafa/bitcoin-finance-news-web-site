    ...
    $.ajax('/Blog/getMyPosts', {
        ...
        success: function (result) {
            ...
            $.each(result, function (i, item) {
                ...
                tr += "<td>";
                tr += "<a href='/makale-duzenle/" + item.ID +"' target='_blank' title='Düzenle' class='btn btn-xs btn-primary' >  <i class='fa fa-edit'> </i> </a>";

                if (item.IsPublish == true) {
                    tr += "<button type='button' class='btn btn-xs btn-danger' title='Yayından Kaldır' onclick='unPublishPost("+item.ID+")' >  <i class='fa fa-remove'> </i> </button>";
                } else {
                    tr += "<button type='button' class='btn btn-xs btn-warning'  title='Yayına Al' onclick='publishPost(" + item.ID +")' >  <i class='fa fa-check-circle'> </i> </button>";
                }
                tr += "</td>";
